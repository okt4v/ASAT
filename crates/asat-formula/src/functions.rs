use crate::evaluator::{to_bool, to_number, to_text, EvalContext, Evaluator};
...
fn fn_power(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    if let Some((n, e)) = first_two_nums(args, ctx, ev) {
        CellValue::Number(n.powf(e))
    } else {
        CellValue::Error(CellError::Value)
    }
}
...
fn fn_log(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let n = match to_number(&ev.eval(&args[0], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let base = if args.len() > 1 {
        to_number(&ev.eval(&args[1], ctx)).unwrap_or(10.0)
    } else {
        10.0
    };
...
fn fn_left(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let n = if args.len() > 1 {
        to_number(&ev.eval(&args[1], ctx)).unwrap_or(1.0) as usize
    } else {
        1
    };
...
fn fn_large(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let mut nums: Vec<f64> = ev
        .expand_range(&args[0], ctx)
        .into_iter()
        .filter_map(|v| to_number(&v))
        .collect();
...
fn fn_median(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    let mut nums: Vec<f64> = expand_args(args, ctx, ev)
        .into_iter()
        .filter_map(|v| to_number(&v))
        .collect();
    if nums.is_empty() {
...
fn fn_stdev(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    let nums: Vec<f64> = expand_args(args, ctx, ev)
        .into_iter()
        .filter_map(|v| to_number(&v))
        .collect();
    if nums.len() < 2 {
...
fn fn_pv(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let rate = match to_number(&ev.eval(&args[0], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let nper = match to_number(&ev.eval(&args[1], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let pmt = match to_number(&ev.eval(&args[2], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let fv = args
        .get(3)
        .and_then(|a| to_number(&ev.eval(a, ctx)))
        .unwrap_or(0.0);
    let typ = args
        .get(4)
        .and_then(|a| to_number(&ev.eval(a, ctx)))
...
fn fn_pmt(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let pv = match to_number(&ev.eval(&args[2], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let fv = args
...
fn fn_nper(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let pmt = match to_number(&ev.eval(&args[1], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let pv = match to_number(&ev.eval(&args[2], ctx)) {
...
fn fn_rate(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let nper = match to_number(&ev.eval(&args[0], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let pmt = match to_number(&ev.eval(&args[1], ctx)) {
    ...
    for _ in 0..200 {
        ...
        if (new_rate - rate).abs() < 1e-10 {
            return CellValue::Number(new_rate);
        }
        rate = new_rate;
        if rate <= -1.0 {
            return CellValue::Error(CellError::Num);
...
fn fn_mirr(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let cfs: Vec<f64> = ev
        ...
        .filter_map(|v| to_number(&v))
        .collect();
    let fin_r = match to_number(&ev.eval(&args[1], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let reinv_r = match to_number(&ev.eval(&args[2], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let n = cfs.len();
...
fn fn_ipmt(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let per = match to_number(&ev.eval(&args[1], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let nper = match to_number(&ev.eval(&args[2], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let pv = match to_number(&ev.eval(&args[3], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let fv = args
    ...
    let pmt = if rate == 0.0 {
        ...
        let rn = r1n(rate, nper);
        -(pv * rn + fv) * rate / ((rn - 1.0) * (1.0 + rate * typ))
    };
    let k = per - 1.0 + typ;
    let bal = if rate == 0.0 {
        pv + pmt * k
    } else {
        pv * r1n(rate, k) + pmt * (r1n(rate, k) - 1.0) / rate
    };
...
fn fn_ppmt(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let pmt_args: Vec<Expr> = [0, 2, 3, 4, 5]
        .iter()
...
fn fn_sln(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let cost = match to_number(&ev.eval(&args[0], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let salvage = match to_number(&ev.eval(&args[1], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let life = match to_number(&ev.eval(&args[2], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    if life == 0.0 {
...
fn fn_ddb(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let period = match to_number(&ev.eval(&args[3], ctx)) {
        Some(v) => v as usize,
        None => return CellValue::Error(CellError::Value),
    };
    let factor = args
...
fn fn_effect(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let nom = match to_number(&ev.eval(&args[0], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let npery = match to_number(&ev.eval(&args[1], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    if npery < 1.0 || nom <= 0.0 {
        return CellValue::Error(CellError::Num);
    }
...
fn fn_nominal(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let eff = match to_number(&ev.eval(&args[0], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    let npery = match to_number(&ev.eval(&args[1], ctx)) {
...
fn fn_cumipmt(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let start = match to_number(&ev.eval(&args[3], ctx)) {
        Some(v) => v as usize,
        None => return CellValue::Error(CellError::Value),
    };
    let end = match to_number(&ev.eval(&args[4], ctx)) {
        Some(v) => v as usize,
        None => return CellValue::Error(CellError::Value),
    };
    let typ = match to_number(&ev.eval(&args[5], ctx)) {
        Some(v) => v,
        None => return CellValue::Error(CellError::Value),
    };
    if start < 1 || end > nper || start > end {
        return CellValue::Error(CellError::Num);
    }
    ...
    let pmt = if rate == 0.0 {
        ...
        -(pv * rn) * rate / ((rn - 1.0) * (1.0 + rate * typ))
    };
    let mut total = 0.0;
...
fn fn_rank(args: &[Expr], ctx: &EvalContext<'_>, ev: &Evaluator) -> CellValue {
    ...
    let rank = vals
        .iter()
        .filter(|&&v| if ascending { v < num } else { v > num })
        .count()
        + 1;
