...
const UNIX_DAYS: &[i64] = &[
    20090, 20091, 20094, 20095, 20096, 20097, 20098, 20101, 20102, 20103, 20104, 20105, 20109,
    20110, 20111, 20112, 20115, 20116, 20117, 20118,
];
...
fn build_price_sheet(sheet: &mut Sheet) {
    ...
    for (col, (label, align)) in hdrs.iter().zip(aligns).enumerate() {
        sheet.set_cell(
            1,
            col as u32,
            Cell::with_style(CellValue::Text((*label).into()), hdr(align)),
        );
    }
    ...
    sheet.notes.insert(
        (1, 4),
        "Close price — used for all return calculations".into(),
    );
    sheet
        .notes
        .insert((1, 5), "Shares traded — formatted as #,##0".into());
    ...
    sheet
        .notes
        .insert((16, 4), "Period high close: $604.88 on Jan 21".into());
    // Note on lowest close
    sheet
        .notes
        .insert((8, 4), "Period low close: $576.98 on Jan 10".into());
}
...
fn build_returns_sheet(sheet: &mut Sheet) {
    ...
    sheet.notes.insert(
        (1, 2),
        "Daily Return = (Close_n - Close_{n-1}) / Close_{n-1}".into(),
    );
    sheet.notes.insert(
        (1, 3),
        "Simple moving average of last 5 closing prices".into(),
    );
    sheet.notes.insert(
        (1, 4),
        "Simple moving average of last 10 closing prices".into(),
    );
...
fn build_summary_sheet(sheet: &mut Sheet) {
    ...
    let kv = |sheet: &mut Sheet, r: u32, label: &str, formula: &str, fmt: Option<NumberFormat>| {
        sheet.set_cell(
            r,
            0,
            Cell::with_style(CellValue::Text(label.into()), label_style()),
        );
        sheet.set_cell(
            r,
            1,
    ...
    kv(
        ...
        "  Start Date",
        "'Price Data'!A3",
        Some(NumberFormat::Date(String::new())),
    );
    row += 1;
    kv(
        sheet,
    ...
    kv(
        ...
        row,
        "  Trading Days",
        "COUNT('Price Data'!E3:E22)",
        None,
    );
    row += 1;

    row += 1; // spacer
    sec_hdr(sheet, row, "PRICE PERFORMANCE");
    row += 1;
    kv(
        sheet,
        row,
        "  Start Price (Jan 2)",
        "'Price Data'!E3",
        Some(NumberFormat::Currency("$".into())),
    );
    row += 1;
    kv(
        sheet,
    ...
    kv(
        ...
        Some(NumberFormat::Percentage(2)),
    );
    row += 1;
    kv(
        sheet,
    ...
    row += 1; // spacer
    sec_hdr(sheet, row, "RETURN STATISTICS");
    row += 1;
    kv(
        sheet,
        row,
        "  Avg Daily Return",
        "AVERAGE(Returns!C4:C22)",
        Some(NumberFormat::Percentage(3)),
    );
    row += 1;
    kv(
        sheet,
    ...
    kv(
        ...
        Some(NumberFormat::Percentage(1)),
    );
    row += 1;

    row += 1; // spacer
    sec_hdr(sheet, row, "VOLUME & BREADTH");
    row += 1;
    kv(
        sheet,
        row,
        "  Avg Daily Volume",
        "AVERAGE('Price Data'!F3:F22)",
        Some(NumberFormat::Thousands),
    );
    row += 1;
    kv(
        sheet,
    ...
    sheet
        .notes
        .insert((6, 1), "First close price on Jan 2 2025".into());
    sheet
        .notes
        .insert((7, 1), "Last close price on Jan 30 2025".into());
    sheet.notes.insert(
        (15, 1),
        "STDEV of daily returns * √252 trading days/year".into(),
    );
}
...
fn main() {
    ...
    println!(
        "  Sheet 1: Price Data   — {} cells",
        wb.sheets[0].cells.len()
    );
    println!(
        "  Sheet 2: Returns      — {} cells",
        wb.sheets[1].cells.len()
    );
    println!(
        "  Sheet 3: Summary      — {} cells",
        wb.sheets[2].cells.len()
    );
}
