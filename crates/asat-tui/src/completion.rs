...
pub fn render(frame: &mut Frame, screen: Rect, state: &RenderState<'_>) {
    ...
    if cmd_buf.contains(' ') {
        ...
        if !subcmds.is_empty() {
            render_subcmd_popup(frame, screen, state, subcmds, state.input.subcmd_completion_idx);
        }
        return;
    }
    ...
    // ── Sizing ────────────────────────────────────────────────────────────
    let cmd_col_w = visible.iter()
        .map(|(_, (cmd, _))| cmd.len())
        .max()
        .unwrap_or(4) as u16;
    let desc_col_w = visible.iter()
    ...
    let popup_rect = Rect { x, y, width: popup_w.min(screen.width.saturating_sub(x)), height: popup_h };
    ...
    let cmd_style      = Style::default().fg(cursor_bg).bg(bg).add_modifier(Modifier::BOLD);
    let desc_style     = Style::default().fg(header_fg).bg(bg);
    let sel_cmd_style  = Style::default().fg(cursor_bg).bg(sel_bg).add_modifier(Modifier::BOLD);
    ...
    let lines: Vec<Line> = visible.iter().map(|(local_idx, (cmd, desc))| {
        ...
        let padding   = " ".repeat(
            (max_cmd_w as usize).saturating_sub(cmd_txt.len())
        );
    ...
    let title = if overflow > 0 {
        ...
        format!(" {} completion{} ", completions.len(), if completions.len() == 1 { "" } else { "s" })
    };
...
fn render_subcmd_popup(
    ...
    if popup_rect.width < 6 || popup_rect.height < 3 { return; }
    ...
    let lines: Vec<Line> = visible.iter().map(|(local_idx, name)| {
        let is_sel = selected.map(|s| s == *local_idx).unwrap_or(false);
        if is_sel {
            Line::from(vec![
                Span::styled(" ▶ ", Style::default().fg(cursor_bg).bg(sel_bg)),
                Span::styled(name.as_str(), Style::default().fg(sel_fg).bg(sel_bg).add_modifier(Modifier::BOLD)),
