...
impl FileDriver for AsatDriver {
    fn read(&self, path: &Path) -> Result<Workbook, IoError> {
        ...
        if version != ASAT_VERSION {
            return Err(IoError::Codec(format!("unsupported ASAT version {}", version)));
        }

        let decompressed = zstd::decode_all(&compressed[8..])
            .map_err(|e| IoError::Codec(e.to_string()))?;

        let mut wb: Workbook = bincode::deserialize(&decompressed)
            .map_err(|e| IoError::Codec(e.to_string()))?;
    ...
    fn write(&self, workbook: &Workbook, path: &Path) -> Result<(), IoError> {
        let serialized = bincode::serialize(workbook)
            .map_err(|e| IoError::Codec(e.to_string()))?;
