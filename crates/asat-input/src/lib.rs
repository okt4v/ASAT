...
pub fn get_command_completions(prefix: &str) -> Vec<(&'static str, &'static str)> {
    ...
    EX_COMMANDS.iter()
        .filter(|(cmd, _)| {
            ...
            let word = cmd.split(|c: char| c == ' ' || c == '<').next().unwrap_or(cmd);
...
pub enum Mode {
    Normal,
    Insert { replace: bool },
    Visual { block: bool },   // v = char visual, Ctrl+V = column/block
    ...
    FormulaSelect { anchor: Option<(u32, u32)> },
}
...
impl Mode {
    pub fn name(&self) -> &str {
        match self {
            ...
            Mode::Recording { register }    => Box::leak(format!("REC({})", register).into_boxed_str()),
...
impl Cursor {
    pub fn new() -> Self { Cursor { row: 0, col: 0 } }
}
...
pub enum AppAction {
    // ── Navigation ──
    MoveCursor { row_delta: i32, col_delta: i32 },
    MoveCursorTo { row: u32, col: u32 },
    MoveToFirstRow,
    MoveToLastRow,
    MoveToFirstCol,
    MoveToLastCol,
    ...
    MoveToNextNonEmptyH { forward: bool },
    /// Jump to next/prev non-empty cell vertically (W / B)
    MoveToNextNonEmptyV { forward: bool },
    /// Jump to next/prev "paragraph" (empty row boundary) (} / {)
    MoveToNextParagraph { forward: bool },
    ...
    // ── Mode transitions ──
    EnterInsert { replace: bool },
    EnterVisual { block: bool },
    EnterVisualLine,
    EnterCommand,
    ...
    DeleteCellContent,
    DeleteCellRange { row_start: u32, col_start: u32, row_end: u32, col_end: u32 },
    ...
    InsertColRight,
    DeleteCurrentCol,
    IncreaseColWidth { col: u32 },  // >>
    DecreaseColWidth { col: u32 },  // <<
    AutoFitCol { col: u32 },        // =
    IncreaseRowHeight { row: u32 }, // +
    DecreaseRowHeight { row: u32 }, // -
    AutoFitRow { row: u32 },        // _

    // ── Yank / paste ──
    YankRow,
    YankCell,    // yc — yank single cell to register + system clipboard
    YankCellRange { row_start: u32, col_start: u32, row_end: u32, col_end: u32, is_line: bool },
    PasteAfter,
    PasteBefore,
    ...
    JumpToCol { col: u32 },   // f{A-Z} — jump to column by letter
    ...
    InsertSumBelow { row_start: u32, col_start: u32, row_end: u32, col_end: u32 },  // S in visual
...
impl InputState {
    ...
    pub fn search_highlight(&self, row: u32, col: u32) -> Option<bool> {
        let pos = self.search_matches.iter().position(|&(r, c)| r == row && c == col)?;
    ...
    pub fn key_prefix(&self) -> String {
        self.key_buffer.iter().map(|k| key_event_to_str(k)).collect()
    }
    ...
    fn handle_normal(&mut self, key: KeyEvent, workbook: &Workbook) -> Vec<AppAction> {
        ...
        if !self.key_buffer.is_empty() {
            ...
            return match (first, key.code) {
                ...
                (KeyCode::Char('>'), KeyCode::Char('>')) => {
                    ...
                    (0..n).map(|_| AppAction::IncreaseColWidth { col: self.cursor.col }).collect()
                }
                (KeyCode::Char('<'), KeyCode::Char('<')) => {
                    (0..n).map(|_| AppAction::DecreaseColWidth { col: self.cursor.col }).collect()
                }
                ...
                (KeyCode::Char('@'), KeyCode::Char('@')) => {
                    ...
                    (0..n).map(|_| AppAction::PlayMacro { register: reg }).collect()
                }
                (KeyCode::Char('@'), KeyCode::Char(c)) if c.is_ascii_alphabetic() => {
                    (0..n).map(|_| AppAction::PlayMacro { register: c }).collect()
                }
        ...
        match key.code {
            ...
            KeyCode::Char('h') | KeyCode::Left  =>
                (0..n).map(|_| AppAction::MoveCursor { row_delta: 0, col_delta: -1 }).collect(),
            KeyCode::Char('j') | KeyCode::Down  =>
                (0..n).map(|_| AppAction::MoveCursor { row_delta: 1, col_delta: 0 }).collect(),
            ...
            // Word/block jumps
            KeyCode::Char('w') => (0..n).map(|_| AppAction::MoveToNextNonEmptyH { forward: true }).collect(),
            KeyCode::Char('b') => (0..n).map(|_| AppAction::MoveToNextNonEmptyH { forward: false }).collect(),
            KeyCode::Char('W') => (0..n).map(|_| AppAction::MoveToNextNonEmptyV { forward: true }).collect(),
            KeyCode::Char('B') => (0..n).map(|_| AppAction::MoveToNextNonEmptyV { forward: false }).collect(),
            KeyCode::Char('e') => (0..n).map(|_| AppAction::MoveToNextNonEmptyH { forward: true }).collect(),

            // Paragraph jumps
            KeyCode::Char('}') => (0..n).map(|_| AppAction::MoveToNextParagraph { forward: true }).collect(),
            KeyCode::Char('{') => (0..n).map(|_| AppAction::MoveToNextParagraph { forward: false }).collect(),
            ...
            KeyCode::Char('g') => { self.key_buffer.push(key); vec![AppAction::NoOp] }
            ...
            // z prefix — scroll
            KeyCode::Char('z') => { self.key_buffer.push(key); vec![AppAction::NoOp] }

            // ── Tab / sheet ───────────────────────────────────────────────────
            KeyCode::Tab     => (0..n).map(|_| AppAction::MoveCursor { row_delta: 0, col_delta: 1 }).collect(),
            KeyCode::BackTab => (0..n).map(|_| AppAction::MoveCursor { row_delta: 0, col_delta: -1 }).collect(),
            KeyCode::Char('t') if ctrl => vec![AppAction::NextSheet],
            KeyCode::Char('T') if ctrl => vec![AppAction::PrevSheet],

            // ── Editing ───────────────────────────────────────────────────────
            KeyCode::Enter | KeyCode::F(2) => {
                let val = workbook.active().get_value(self.cursor.row, self.cursor.col);
            ...
            // d prefix — dd or bail
            KeyCode::Char('d') if !ctrl => { self.key_buffer.push(key); vec![AppAction::NoOp] }
            ...
            // = — auto-fit column
            KeyCode::Char('=') => vec![AppAction::AutoFitCol { col: self.cursor.col }],

            // + / - — row height; _ — auto-fit (reset) row
            KeyCode::Char('+') =>
                (0..n).map(|_| AppAction::IncreaseRowHeight { row: self.cursor.row }).collect(),
            KeyCode::Char('-') =>
                (0..n).map(|_| AppAction::DecreaseRowHeight { row: self.cursor.row }).collect(),
            KeyCode::Char('_') => vec![AppAction::AutoFitRow { row: self.cursor.row }],

            // f prefix — column jump by letter
            KeyCode::Char('f') if !ctrl => { self.key_buffer.push(key); vec![AppAction::NoOp] }

            // J — join: concat cell below into current cell, then clear below
            KeyCode::Char('J') => vec![AppAction::JoinCellBelow],

            // > / < prefix — column width
            KeyCode::Char('>') => { self.key_buffer.push(key); vec![AppAction::NoOp] }
            KeyCode::Char('<') => { self.key_buffer.push(key); vec![AppAction::NoOp] }

            // ── Yank / paste ──────────────────────────────────────────────────
            KeyCode::Char('y') => { self.key_buffer.push(key); vec![AppAction::NoOp] }
            KeyCode::Char('p') => (0..n).map(|_| AppAction::PasteAfter).collect(),
            KeyCode::Char('P') => (0..n).map(|_| AppAction::PasteBefore).collect(),

            // ── Visual modes ──────────────────────────────────────────────────
            KeyCode::Char('v') if !ctrl => {
                self.visual_anchor = Some(VisualAnchor { row: self.cursor.row, col: self.cursor.col });
            ...
            // ── Marks ─────────────────────────────────────────────────────────
            KeyCode::Char('m') => { self.key_buffer.push(key); vec![AppAction::NoOp] }
            KeyCode::Char('\'') => { self.key_buffer.push(key); vec![AppAction::NoOp] }

            // ── Macro recording / playback ─────────────────────────────────────
            // q alone buffers; q{char} starts recording (handled in multi-key block above)
            KeyCode::Char('q') if !ctrl => { self.key_buffer.push(key); vec![AppAction::NoOp] }
            // @ alone buffers; @{char} plays macro (handled in multi-key block above)
            KeyCode::Char('@') if !ctrl => { self.key_buffer.push(key); vec![AppAction::NoOp] }

            KeyCode::Esc => { self.count_buffer.clear(); vec![AppAction::NoOp] }
            _ => vec![AppAction::NoOp],
        }
    }
    ...
    fn handle_formula_select(&mut self, key: KeyEvent) -> Vec<AppAction> {
        ...
        match key.code {
            ...
            KeyCode::Char('h') | KeyCode::Left  =>
                vec![AppAction::MoveCursor { row_delta: 0, col_delta: -1 }],
            KeyCode::Char('j') | KeyCode::Down  =>
                vec![AppAction::MoveCursor { row_delta: 1, col_delta: 0 }],
            KeyCode::Char('k') | KeyCode::Up    =>
                vec![AppAction::MoveCursor { row_delta: -1, col_delta: 0 }],
            KeyCode::Char('l') | KeyCode::Right =>
                vec![AppAction::MoveCursor { row_delta: 0, col_delta: 1 }],
            ...
            KeyCode::Char('g') if !ctrl => { self.key_buffer.push(key); vec![AppAction::NoOp] }
            KeyCode::Char('G') => vec![AppAction::MoveToLastRow],
    ...
    fn handle_insert(&mut self, key: KeyEvent, _replace: bool) -> Vec<AppAction> {
        ...
        match key.code {
            ...
            KeyCode::Char('w') if ctrl => {
                if self.edit_cursor_pos > 0 {
                    ...
                    while pos > 0 {
                        ...
                        if self.edit_buffer[prev..pos].chars().next()
                            .map(|c| c.is_whitespace()).unwrap_or(false) {
                            pos = prev;
                        } else {
                            break;
                        }
            ...
            KeyCode::Esc if !ctrl => {
                ...
                self.edit_buffer.clear();
                self.edit_cursor_pos = 0;
                vec![AppAction::SetCell { sheet: 0, row, col, value }, AppAction::ExitMode]
            }
            KeyCode::Enter => {
                let value = parse_cell_value(&self.edit_buffer);
                let row = self.cursor.row;
                ...
                self.edit_cursor_pos = 0;
                vec![
                    AppAction::SetCell { sheet: 0, row, col, value },
                    AppAction::ExitMode,
                    AppAction::MoveCursor { row_delta: 1, col_delta: 0 },
                ]
            }
            KeyCode::Tab => {
                let value = parse_cell_value(&self.edit_buffer);
                ...
                vec![
                    AppAction::SetCell { sheet: 0, row, col, value },
                    AppAction::ExitMode,
                    AppAction::MoveCursor { row_delta: 0, col_delta: 1 },
                ]
            }
            KeyCode::Backspace => {
                if self.edit_cursor_pos > 0 {
            ...
            KeyCode::Left  => {
                if self.edit_cursor_pos > 0 {
                    self.edit_cursor_pos = prev_char_boundary(&self.edit_buffer, self.edit_cursor_pos);
                }
                vec![AppAction::NoOp]
            }
            KeyCode::Right => {
                if self.edit_cursor_pos < self.edit_buffer.len() {
                    self.edit_cursor_pos = next_char_boundary(&self.edit_buffer, self.edit_cursor_pos);
                }
                vec![AppAction::NoOp]
            }
    ...
    fn handle_visual(&mut self, key: KeyEvent, is_line: bool) -> Vec<AppAction> {
        match key.code {
            ...
            // Toggle modes
            KeyCode::Char('v') if !is_line => { self.visual_anchor = None; vec![AppAction::ExitMode] }
            KeyCode::Char('V') if  is_line => { self.visual_anchor = None; vec![AppAction::ExitMode] }
            KeyCode::Char('V') if !is_line => vec![AppAction::EnterVisualLine],
            KeyCode::Char('v') if  is_line => vec![AppAction::EnterVisual { block: false }],

            // Movement
            KeyCode::Char('h') | KeyCode::Left  => vec![AppAction::MoveCursor { row_delta: 0, col_delta: -1 }],
            KeyCode::Char('j') | KeyCode::Down  => vec![AppAction::MoveCursor { row_delta: 1, col_delta: 0 }],
            KeyCode::Char('k') | KeyCode::Up    => vec![AppAction::MoveCursor { row_delta: -1, col_delta: 0 }],
            KeyCode::Char('l') | KeyCode::Right => vec![AppAction::MoveCursor { row_delta: 0, col_delta: 1 }],
        ...
        match key.code {
            ...
            KeyCode::Tab | KeyCode::BackTab => {
                ...
                if self.completion_idx.is_none() {
                    self.completion_prefix = self.command_buffer
                        .split_whitespace()
                        .next()
                        .unwrap_or("")
                        .to_string();
                ...
                let word = chosen.split(|c: char| c == ' ' || c == '<')
                    .next().unwrap_or(chosen);
    ...
    fn handle_search(&mut self, key: KeyEvent, forward: bool) -> Vec<AppAction> {
        match key.code {
            KeyCode::Esc       => { self.search_buffer.clear(); vec![AppAction::ExitMode] }
            KeyCode::Enter     => {
            ...
            KeyCode::Backspace => { self.search_buffer.pop(); vec![AppAction::NoOp] }
            KeyCode::Char(c)   => { self.search_buffer.push(c); vec![AppAction::NoOp] }
            _ => vec![AppAction::NoOp],
        }
    }
}
...
pub fn parse_cell_value(s: &str) -> CellValue {
    ...
    if let Some(f) = s.strip_prefix('=') { return CellValue::Formula(f.to_string()); }
