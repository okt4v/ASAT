...
const MENU: &[MenuItem] = &[
    MenuItem { key: "n", icon: "◆", name: "New File",      desc: "Start with a blank workbook"              },
    MenuItem { key: "f", icon: "◆", name: "Find File",     desc: "Fuzzy-search files in current directory"  },
    MenuItem { key: "r", icon: "◆", name: "Recent Files",  desc: "Browse recently opened files"             },
    MenuItem { key: "t", icon: "◆", name: "Themes",        desc: "Browse and apply colour themes"           },
    MenuItem { key: "c", icon: "◆", name: "Edit Config",   desc: "Open config.toml in $EDITOR"             },
    MenuItem { key: "q", icon: "◆", name: "Quit",          desc: "Exit ASAT"                                },
];
...
pub fn render_welcome(frame: &mut Frame, area: Rect, state: &RenderState<'_>) {
    ...
    let logo_width = LOGO.iter().map(|l| UnicodeWidthStr::width(*l) as u16).max().unwrap_or(30);
    ...
    for (i, line) in LOGO.iter().enumerate() {
        ...
        if y >= logo_area.y + logo_area.height { break; }
        let rect = Rect { x: center_x, y, width: logo_width, height: 1 };
        frame.render_widget(
            Paragraph::new(Line::from(Span::styled(*line, Style::default().fg(amber).add_modifier(Modifier::BOLD)))),
            rect,
        );
    }
    ...
    frame.render_widget(
        Paragraph::new(Line::from(vec![
            Span::styled("Advanced Spreadsheet Alteration Tool", Style::default().fg(header_fg)),
        ])).alignment(Alignment::Center),
        rows[2],
    );
    frame.render_widget(
        Paragraph::new(Line::from(vec![
            Span::styled("vim-powered  ·  terminal-native  ·  written in rust",
    ...
    frame.render_widget(
        ...
        Rect { x: div_x, y: rows[5].y, width: div_w, height: 1 },
    );

    // ── Menu items ────────────────────────────────────────────────────────
    let menu_w    = 52u16.min(area.width);
    let menu_x    = area.x + area.width.saturating_sub(menu_w) / 2;
    let menu_area = Rect { x: menu_x, y: rows[6].y, width: menu_w, height: rows[6].height };

    for (i, item) in MENU.iter().enumerate() {
        let y = menu_area.y + i as u16;
        if y >= menu_area.y + menu_area.height { break; }
        let row = Rect { x: menu_x, y, width: menu_w, height: 1 };

        let is_quit = item.key == "q";
        let key_color  = if is_quit { darken(header_fg, 0.75) } else { amber };
        let name_color = if is_quit { darken(header_fg, 0.65) } else { Color::White };
        ...
        frame.render_widget(
            Paragraph::new(Line::from(vec![
                ...
                Span::styled(format!("{:<18}", item.name), Style::default().fg(name_color).add_modifier(Modifier::BOLD)),
    ...
    frame.render_widget(
        Paragraph::new(Line::from(vec![
            ...
            Span::styled("  ·  press ? for help", Style::default().fg(darken(header_fg, 0.4))),
        ])).alignment(Alignment::Center),
        rows[8],
    );
}
...
pub fn render_file_finder(frame: &mut Frame, area: Rect, state: &RenderState<'_>) {
    ...
    let panel_w = (area.width * 3 / 4).max(50).min(area.width.saturating_sub(4));
    let panel_h = (area.height * 3 / 4).max(12).min(area.height.saturating_sub(4));
    let panel_x = area.x + (area.width - panel_w) / 2;
    let panel_y = area.y + (area.height - panel_h) / 2;
    let panel   = Rect { x: panel_x, y: panel_y, width: panel_w, height: panel_h };
    ...
    if files.is_empty() {
        let empty = Paragraph::new(Line::from(Span::styled(
            if query.is_empty() { " No files found in current directory" }
            else { " No files match your query" },
        ...
        let hint_block = results_block.title_bottom(
            Span::styled(hint, Style::default().fg(darken(header_fg, 0.55))),
        );
        frame.render_widget(Clear, inner[1]);
...
pub fn render_recent_files(frame: &mut Frame, area: Rect, state: &RenderState<'_>) {
    ...
    let sel_bg    = parse_hex_color(&theme.selection_bg);
    let bg        = parse_hex_color(&theme.cell_bg);
    let sel_fg    = if is_dark_color(sel_bg) { Color::White } else { Color::Black };

    frame.render_widget(Paragraph::new("").style(Style::default().bg(bg)), area);

    let panel_w = (area.width * 2 / 3).max(50).min(area.width.saturating_sub(4));
    ...
    if files.is_empty() {
        let para = Paragraph::new(vec![
            Line::from(""),
            Line::from(Span::styled("  No recent files yet.", Style::default().fg(darken(header_fg, 0.6)))),
            Line::from(Span::styled("  Open a file with :e <path> to get started.", Style::default().fg(darken(header_fg, 0.45)))),
        ]).block(block);
    ...
    let items: Vec<ListItem> = files.iter()
        ...
        .map(|(i, path)| {
            ...
            if is_sel {
                ListItem::new(Line::from(vec![
                    Span::styled(" ▶ ", Style::default().fg(amber)),
                    Span::styled(name, Style::default().fg(sel_fg).add_modifier(Modifier::BOLD)),
                    Span::styled(format!("  {}", parent), Style::default().fg(darken(sel_fg, 0.6))),
                ])).style(Style::default().bg(sel_bg))
            } else {
                ListItem::new(Line::from(vec![
                    Span::styled("   ", Style::default()),
                    Span::styled(name, Style::default().fg(header_fg)),
                    Span::styled(format!("  {}", parent), Style::default().fg(darken(header_fg, 0.55))),
                ]))
            }
        })
        .collect();
